...
class fpdfHelper extends FPDF {
    ...
    function setparametrosTotalFinal($valor,$etiqueta)
    {
    	...
    	$this->etiquetaTotalFinal = $etiqueta;
    }
    ...
    // funcion que establece la cabecera del reporte
	function Header()
    {	    	
        if($this->rutaImagen)
        {	//define el logo del reporte        
        	$this->Image($this->rutaImagen,10,2,12);						// define el logo del reporte
        } else {
        	$this->SetFont('Arial','',self::TAMANIO_LETRA - 1);			// defiene el formato para la fecha y hora        
        	$this->Cell(0,0,$this->tituloEncabezadoIzquierdo,0,0,'L'); // hora y fecha de impresion       	
        }        
        $this->SetFont('Arial','',self::TAMANIO_LETRA - 1);			// defiene el formato para la fecha y hora        
        $this->Cell(0,0,'Fecha/hora '.date('d/m/Y H:i:s'),0,0,'R'); // hora y fecha de impresion       
        $this->Ln(self::SALTO_LINEA + 2);							// salto de linea
        $this->line($this->GetX(),$this->GetY()-1,205 + $this->orientacionPapel,$this->GetY()-1);	// define la linea de division
    }

    // funcion que establece el Pie de pagina
    function Footer()
    {        
        $this->SetY(-15);		// posicion 1.5 cm bajo el reporte
        $this->SetFont('Arial','',self::TAMANIO_LETRA-1);	
        $this->line($this->GetX(),$this->GetY(),205 + $this->orientacionPapel,$this->GetY());
        $this->Cell(0,10,utf8_decode('Pag ').$this->PageNo().' de  {nb}',0,0,'R');	// numero de pagina
    }
    ...
    function cabeceraAgrupada($cabeceraAgrupada) {
    	...
    	$this->saltoDeLinea();
		$this->SetFont('Arial','BU',self::TAMANIO_LETRA + 3);
		...
		if(is_array($cabeceraAgrupada)) {
			foreach($cabeceraAgrupada as $id => $valor) {
				...
				$this->SetFont('Arial','B',self::TAMANIO_LETRA);
				$this->Cell(40,0,utf8_decode($valor['0']),0,0,'L');
				$this->SetFont('Arial','',self::TAMANIO_LETRA);
	...
	{	
		...
		if($this->subtitulo <> '')
		{
			if(is_array($this->subtitulo))
			{
				...
				{
					//sub titulo del documento
					$this->SetFont('Arial',($subtituloEnNegrita ? 'B' : ''),self::TAMANIO_LETRA);
        			$this->Cell(40,0,utf8_decode($valor['0']),0,0,'L');
					$this->SetFont('Arial','',self::TAMANIO_LETRA);
        			$this->Cell($this->GetX(),0,utf8_decode($valor['1']),0,0,'L');
        			$this->Ln(self::SALTO_LINEA + 1);
    	...
    	$this->SetFont('Arial','B',self::TAMANIO_LETRA);		// define el formato de texto para le titulo    	
    	...
    	$this->SetFont('Arial','',$tamanioLetra);	// defiene el formato del texto para el cuerpo del reporte
    	else
    	$this->SetFont('Arial','',self::TAMANIO_LETRA);	// defiene el formato del texto para el cuerpo del reporte
			...
			{	
				...
				{	
					...
					{	// si existe asume que no se debe mostrar en el reporte
						...
						$this->SetFont('Arial','B',self::TAMANIO_LETRA);	// defiene el formato del texto para el cuerpo del reporte		
					...
					{	
						...
						{	
							...
							{	// si existe asume que no se debe mostrar en el reporte
								...
								$this->SetFont('Arial','B',self::TAMANIO_LETRA);	// defiene el formato del texto para el cuerpo del reporte		
								$this->Cell($arr_tamanio_celda[$capturaCampo],7,utf8_decode($this->etiquetaTotalFinal),0,0,$arr_alineacion[$capturaCampo],0);
							}
						}	
					}
				}
