...
/*
$Id: nusoap.php,v 1.1.1.1 2007/04/14 14:28:46 rossoft Exp $
...
* @version  $Id: nusoap.php,v 1.1.1.1 2007/04/14 14:28:46 rossoft Exp $
* @access   public
*/
class nusoap_base {
	/**
    ...
    * @access public
    */
    function serializeEnvelopeWithouHeader($body,$headers=false,$namespaces=array(),$style='rpc',$use='encoded',$encodingStyle='http://schemas.xmlsoap.org/soap/encoding/'){
    // TODO: add an option to automatically run utf8_encode on $body and $headers
    // if $this->soap_defencoding is UTF-8.  Not doing this automatically allows
    // one to send arbitrary UTF-8 characters, not just characters that map to ISO-8859-1
...
class soap_server extends nusoap_base {
	...
	*/
	function invoke_method() {
		...
		// does method exist?
		if ($this->operations[$this->methodname]['function'])
		{
			$method=$this->operations[$this->methodname]['function'];
			if (! method_exists($method[0], $method[1]))
			{
				$this->debug("in invoke_method, function '$this->methodname' not found!");
				$this->result = 'fault: method not found';
				$this->fault('SOAP-ENV:Client',"method '$this->methodname' not defined in the object service");
				return;				
			}
			else
			{
				...
				{
					$params=array_values($this->methodparams);				
				}
				else
				{
					$params=array();
				}
				$this->methodreturn = call_user_func_array($method, $params);																
			}
		}
		else
		{
			...
			if (!function_exists('call_user_func_array')) {
				...
				$this->methodreturn = call_user_func_array($call_arg, $params);
	...
	* @access   public
	*/
	function register($name,$function=null,$in=array(),$out=array(),$namespace=false,$soapaction=false,$style=false,$use=false,$documentation='',$encodingStyle=''){
		global $HTTP_SERVER_VARS;
	    ...
	    'style' => $style);
        if($this->wsdl){
        	$this->wsdl->addOperation($name,$function,$in,$out,$namespace,$soapaction,$style,$use,$documentation,$encodingStyle);
	    }
		return true;
	}
...
class wsdl extends nusoap_base {
	...
	* @access public 
	*/
	function addOperation($name,$function, $in = false, $out = false, $namespace = false, $soapaction = false, $style = 'rpc', $use = 'encoded', $documentation = '', $encodingStyle = ''){
		if ($use == 'encoded' && $encodingStyle == '') {
			$encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/';
		}
